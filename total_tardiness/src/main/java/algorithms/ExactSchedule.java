...
public class ExactSchedule {
     ...
     * @param instance
     */
    public ExactSchedule(ProblemInstance instance) {
    ...
    public int getMinimumTardiness(int t) {
        if (n == 0)
            return 0;
...
//        return Util.getTotalTardiness(getOptimalSchedule(t));
    ...
    private void computeDeltas(int i, int j, int k, int t) {
        List<Integer> setS;
        if (k == -1) {
